...
public class UserRepository {
    ...
    /** Returns the account for a username, including the password hash. */
    public Optional<UserAccount> findAccountByUsername(String username) {
        return new Sql().first(SELECT_ACCOUNT, this::mapAccount, username);
    }

    /** Returns the account for an id, including the password hash. */
    public Optional<UserAccount> findAccountById(long userId) {
        return new Sql().first(SELECT_ACCOUNT_BY_ID, this::mapAccount, userId);
    }
    ...
    public byte[] findAvatar(long userId) {
        List<byte[]> found = new Sql().list("SELECT avatar FROM users WHERE id = ?",
    ...
    public void linkByEmailIfUnlinked(long userId) {
        new Sql().executeUpdate(
                "UPDATE users u SET u.employee_id = ("
                        ...
                        + "WHERE u.id = ? AND u.employee_id IS NULL", userId);
    }
    ...
    public Long findUserIdByEmployeeId(long employeeId) {
        return new Sql().first("SELECT id FROM users WHERE employee_id = ?",
    ...
    public List<Long> findRoleIds(long userId) {
        return new Sql().list("SELECT role_id FROM user_roles WHERE user_id = ?",
    ...
    public void touchLastLogin(long userId) {
        new Sql().executeUpdate("UPDATE users SET last_login_at = CURRENT_TIMESTAMP WHERE id = ?", userId);
    }
